...
public class GraphGenerator2 {
    ...
    public void generateGraph() {
        // Create the graph
        GraphDatabaseService graph = new EmbeddedGraphDatabase("coalition-graph-positive");
                    ...
                    for (int j = i + 1; j < mentionedHandles.length; j++) {
                        String mentionedHandle = mentionedHandles[j];
                            ...
                            if (mentionedHandle.compareTo(handle) < 1) {
                                sentimentHandle = handleMap.get(mentionedHandle) + "&&&&" + handleMap.get(handle);
                            }
                            else {
                                sentimentHandle = handleMap.get(handle) + "&&&&" + handleMap.get(mentionedHandle);
                            }
